...
|---|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------|----------|
| S | (GNRC) An attacker uses another client's authentication token. He can then have access (reference) to any of the impersonated client's keys and execute any operation in their name.        | U-2                              | AS3      |
| S | (GNRC) An attacker uses brute-force on the AUTH field of the request until he does not have an AuthenticationError response status to then deduce a valid authentication token.             | M-2                              | AS1      |
...
| S | An attacker uses a weakness or a bug of one authentication method as soon as the authentication method used by Parsec is changed.                                                           | O-12, U-6                        | AS3      |
...
| I | (GNRC) An attacker steals the authentication token from a client's request to then execute any operation in their name.                                                                     | M-3                              | AS4, AS3 |
...
| 2  | The authenticators should only allow authentication tokens that are secure enough that brute-force attacks are not feasible. *(not DRCT)*                                                                                                                                                                  | Not applicable currently as Parsec does not implement the GNRC case.                                                                                                                |
...
| 10 | A set of mutually trusted clients has restricted read-write access to the service IPC endpoint. *(DRCT)*                                                                  | A Parsec Clients Unix group needs to be created for it to exclusively have read-access on the socket.                                                          |
